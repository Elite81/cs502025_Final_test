...
class User(Base):
  ...
  role: Mapped[Role] = mapped_column(SQLAlchemyEnum(Role), nullable=False, default=Role.STUDENT)
...
class Questions(Base):
   ...
   question_type: Mapped[bool] = mapped_column(Boolean, default=False) # MCQ or True/False
   teacher_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True) #Teacher who set the question
   created_at: Mapped[datetime] =  mapped_column(DateTime, default=lambda:datetime.now(timezone.utc))
...
class QuizResult(Base):
   ...
   id: Mapped[int] = mapped_column(primary_key=True)
   quiz_questions_id: Mapped[int] = mapped_column(ForeignKey("quiz_questions.id"), index=True)
   student_id: Mapped[int] = mapped_column(ForeignKey('users.id'), index=True)
   score: Mapped[float] =  mapped_column(Float, default=None)
   remarks: Mapped[str] = mapped_column(String(128))
   taken_on: Mapped[datetime] =  mapped_column(DateTime, default=lambda:datetime.now(timezone.utc))
